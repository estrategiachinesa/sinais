...
export async function generateSimulatedTradingSignal(
  input: SimulatedTradingSignalInput
): Promise<SimulatedTradingSignalOutput> {
  return generateSimulatedTradingSignalFlow(input);
}
...
const generateSimulatedTradingSignalPrompt = ai.definePrompt({
  ...
  prompt: `Based on the selected expiration time of {{{expirationTime}}}, generate a simulated trading signal for the next target time.
...
const generateSimulatedTradingSignalFlow = ai.defineFlow(
  ...
  async input => {
    ...
    const {output} = await generateSimulatedTradingSignalPrompt({
      ...input,
